...
class ItemView:
    ...
    def show_edit_supplier_screen(self):
        supplier_id_str = simpledialog.askstring("Atualizar Fornecedor", "Digite o ID do fornecedor:")
        ...
        if supplier_id_str:
            ...
            if supplier:
                update_window = tk.Toplevel(self.root)
                update_window.title("Atualizar Fornecedor")
                update_window.geometry("300x250")

                ttk.Label(update_window, text="Nome").pack()
                name_entry = ttk.Entry(update_window)
                name_entry.insert(0, supplier["name"])
                name_entry.pack()

                ttk.Label(update_window, text="Contato").pack()
                contact_entry = ttk.Entry(update_window)
                contact_entry.insert(0, supplier["contact"])
                contact_entry.pack()

                ttk.Label(update_window, text="Endereço").pack()
                ...
                ttk.Button(update_window, text="Salvar", command=update_supplier).pack(pady=10)
            else:
